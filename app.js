// v4.0 — Full bug-fix pass. See changelog at bottom - Claude

// ── THE HYBRID PIPE ──────────────────────────────────────────────────────────
// GET for reads (fast, zero CORS issues), POST for writes (unlimited payload)
async function gasCall(payload, isRetry = false) {
  ...
  try {
    ...
    hideLoader();
    if (json.success === false) console.error('API Error [' + payload.action + ']:', json.error);
    return json;
  } catch (e) {
    hideLoader();
...
function dsec(lid, sid, jobs, type) {
  ...
  if (!l||!s) return;
  if (!jobs||!jobs.length) { s.style.display='none'; return; }
  s.style.display='';
...
async function loadAllData() {
  // 1. Serve from cache immediately so the UI is instant
  ...
  if (cached) {
    try {
      ...
      S.clients    = p.clients    || [];
      S.jobs       = p.jobs       || [];
      S.financials = p.financials || [];   // FIX: was missing — financials now restored from cache
      refreshAll();
    } catch(e) {}
  }

  // 2. Fetch fresh data from Cloud and overlay
  const r = await gasCall({ action: 'getAllData' });
  if (r.success) {
    S.clients    = r.clients    || [];
    S.jobs       = r.jobs       || [];
    S.financials = r.financials || [];     // FIX: was missing — financials now loaded from server
    S.biz        = Object.assign(S.biz, r.biz);
    if (r.lists && Object.keys(r.lists).length) {
      S.lists = Object.assign(JSON.parse(JSON.stringify(DL)), r.lists);
    }
    refreshAll();
    localStorage.setItem('smhq_cache', JSON.stringify({
      clients: S.clients, jobs: S.jobs,
      financials: S.financials, lists: S.lists, biz: S.biz
    }));
  } else {
    showToast('⚠️ Cloud sync failed — working from cache');
  }
}
...
async function drainOfflineQueue() {
  try {
    ...
    for (const item of q) {
      try {
        const r = await gasCall(item.payload, true);   // FIX: was missing await — queue never drained
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('scroll').addEventListener('click', e => {
    ...
    if (ladmin) {
      e.stopPropagation();
      const act = ladmin.dataset.ladmin;
      const k   = ladmin.dataset.lk;
      const i   = parseInt(ladmin.dataset.li);
...
function goBack() {
  if (_backLock) return;
  _backLock = true;
  setTimeout(() => _backLock = false, 250);
  if (S.stack.length) {
    const prevView = S.stack.pop();
    navTo(prevView, false);
  } else {
    navTo('dashboard');
  }
}
...
function renderEarnBars() {
  ...
  for (let w=3;w>=0;w--) {
    ...
    const wMonS=wMon.toISOString().split('T')[0];
    const wSunS=wSun.toISOString().split('T')[0];
...
function openProfile(cid) {
  S.curCli=cid;
  const c=getCli(cid);
  const cj=S.jobs.filter(j=>j.Client_ID===cid);
  const ci=S.financials.filter(f=>f.Client_ID===cid);
  const totPaid=ci.filter(f=>f.Status==='Paid').reduce((s,f)=>s+parseMoney(f.Amount),0);
  const totOwed=cj.filter(j=>j.Job_Status==='Completed'&&!isPaidJob(j)).reduce((s,j)=>s+parseMoney(j.Total_Amount),0);
...
function renderProfileJobs(cj) {
  const f=S.profileJobFilter||'all';
  ['all','paid','owed'].forEach(k=>{ const el=$('pf-'+k); if(!el) return; el.style.outline=f===k?'2px solid var(--pink)':''; el.style.transform=f===k?'scale(1.03)':''; });
  ...
  if ($('p-jobs-list')) {
    $('p-jobs-list').innerHTML=filtered.length?filtered.map(profJobRow).join(''):
...
function profJobRow(j) {
  const isPaid    = isPaidJob(j);
  const isSched   = j.Job_Status==='Scheduled';
  const tod       = today();
  const sd        = j.Scheduled_Date;
  const isASAP    = (j.Scheduling_Type==='ASAP' || j.Scheduling_Type==='asap') && !sd;
  ...
  const sp = isPrePaid ?
    ...
    isPaid ?
    `<span class="pill p-green">✅ ${esc(j.Payment_Method||'Paid')}</span>` :
    isSched ?
    (isASAP ? `<span class="pill p-amb">⚡ ASAP</span>` : `<span class="pill p-blue">📅 Booked</span>`) :
...
function jrHTML(j, type) {
  ...
  let pillHtml='';
  if      (type==='sched')   pillHtml=isASAP?`<span class="pill p-amb">⚡ ASAP</span>`:`<span class="pill p-blue">📅 Booked</span>`;
  else if (type==='owed')    pillHtml=`<span class="pill p-red">⏳ Unpaid</span>`;
  else if (type==='fu')      pillHtml=`<span class="pill p-pink">🔔 Follow-Up</span>`;
  else if (type==='review')  pillHtml=`<span class="pill p-amb">⭐ Ask Review</span>`;
  else if (type==='overdue') pillHtml=`<span class="pill p-ora">⚠️ Past Due</span>`;
  else if (type==='unschd')  pillHtml=`<span class="pill p-amb">${isASAP?'⚡ ASAP':esc(j.Scheduling_Type||'TBD')}</span>`;

  let btnHtml='';
  if (type==='sched'||type==='overdue') {
    if (isASAP) btnHtml=`<button class="btn b-sm b-amb" data-action="schedule" data-jid="${esc(j.Job_ID)}">📅 Set Date</button>`;
    else        btnHtml=`<button class="btn b-sm b-bl" data-action="complete" data-jid="${esc(j.Job_ID)}">✅ Done</button>`;
  } else if (type==='owed')   { btnHtml=`<button class="btn b-sm b-g" data-action="paid" data-jid="${esc(j.Job_ID)}">💰 Paid</button>`; }
  else if (type==='fu')       { btnHtml=`<button class="btn b-sm b-s" data-action="clearfu" data-jid="${esc(j.Job_ID)}">✓ Clear</button>`; }
  else if (type==='review')   { btnHtml=`<button class="btn b-sm b-pur" data-action="revreq" data-jid="${esc(j.Job_ID)}">⭐ Requested</button>`; }
  else if (type==='unschd')   { btnHtml=`<button class="btn b-sm b-amb" data-action="schedule" data-jid="${esc(j.Job_ID)}">📅 Set Date</button>`; }

  // FIX: pre-pay indicator was === 'Scheduled' should be, not !== 'Scheduled'
  // A pre-paid job is one that is SCHEDULED and has a payment already on it
...
function startAddClient() {
  S.editCli=null; S.cliStatus='Lead';
  ['ac-first','ac-last','ac-phone','ac-phone2','ac-email','ac-street','ac-city','ac-postal','ac-notes','ac-family','ac-access'].forEach(i=>$(i).value='');
  setCS('Lead'); popLists();
  if ($('ac-savebook')) $('ac-savebook').style.display='';
...
function setCS(v) {
  S.cliStatus=v;
  ['Lead','Active','Inactive'].forEach(s=>{ const el=$('s-'+s.toLowerCase()); if(el) el.classList.toggle('on',s===v); });
}
...
async function submitClient(thenBook=false) {
  ...
  if (btn && btn.tagName==='BUTTON') { btn.disabled=true; btn.classList.add('saving'); btn.textContent='⏳ Saving...'; }
  ...
  const data={
    ...
    Phone2:          $('ac-phone2')?.value.trim()||'',
    Email:           $('ac-email')?.value.trim()||'',
    Street:          $('ac-street')?.value.trim()||'',
    City:            $('ac-city')?.value.trim()||'Georgetown',
    Province:        'ON',
    Postal_Code:     $('ac-postal')?.value.trim().toUpperCase()||'',
    Status:          S.cliStatus,
    Referral_Source: $('ac-ref')?.value||'',
    Family_Details:  $('ac-family')?.value.trim()||'',
    Access_Info:     $('ac-access')?.value.trim()||'',
    Global_Notes:    $('ac-notes')?.value.trim()||''
  };

  if (S.editCli) {
    // Update existing client
    const c=S.clients.find(x=>x.Client_ID===S.editCli);
    ...
    // FIX: single add path — removed the duplicate block that was below this
    data.Client_ID   = 'C'+Date.now();
    data.Created_Date = today();
    S.clients.push(data);
    S.curCli=data.Client_ID;

    // Navigate first so the user doesn't wait
    if (thenBook) { openBookJobForClient(); }
    else          { navTo('clients'); renderCli(); }

    showToast('✓ '+first+' '+last+' added!');

    // FIX: await so we can receive the real server-assigned ID back
    if (!S.isDemo) {
      const r = await gasCall({action:'addClient', ...data});
      if (r && r.Client_ID && r.Client_ID !== data.Client_ID) {
        const idx=S.clients.findIndex(x=>x.Client_ID===data.Client_ID);
        if (idx!==-1) S.clients[idx].Client_ID=r.Client_ID;
        if (S.curCli===data.Client_ID) S.curCli=r.Client_ID;
        // Update any jobs that were booked in the window between add and sync
        S.jobs.forEach(j=>{ if(j.Client_ID===data.Client_ID) j.Client_ID=r.Client_ID; });
      }
    }
...
async function forceAddClient(data) {
  ...
  // FIX: await so real ID is received — same fix as submitClient
  if (!S.isDemo) {
    const r = await gasCall({action:'addClient', ...data});
    if (r && r.Client_ID && r.Client_ID !== data.Client_ID) {
      const idx=S.clients.findIndex(x=>x.Client_ID===data.Client_ID);
  ...
  if (mdb) {
    mdb.innerHTML=`
      <div style="font-size:14px;color:var(--txt2);margin-bottom:16px;line-height:1.5;">
        ...
        <br><br><span style="color:var(--red);font-weight:800;">This cannot be undone.</span>
      </div>
...
async function deleteClient() {
  const cid=S.curCli; if(!cid) return;
  closeMo('m-del');
...
function openBookJobForLead(cid) { S.curCli=cid; openBookJobForClient(); }
...
function setPrice(t) {
  S.priceType=t;
  $('pr-h')?.classList.toggle('on',t==='Hourly');
  $('pr-f')?.classList.toggle('on',t==='Flat');
...
// Calculation for the Book Job screen
function calc() {
  const mockJob={
  ...
  if (hstRow) {
    if (totals.hst===0) { hstRow.style.display='none'; }
    else {
...
function checkTimeConflict() {
  ...
  const close=conflicts.filter(j=>{ const [jh,jm]=j.Time.split(':').map(Number); return Math.abs(jh*60+jm-selMins)<120; });
  ...
  if (isComp) {
    ...
    $('m-job-body').innerHTML=`
      <div style="background:var(--green-s);border:1.5px solid var(--green-b);border-radius:var(--r);padding:14px;margin-bottom:16px;">
        ...
        ${addCost>0?lrow('Additional costs',j.Additional_Cost_Notes?esc(j.Additional_Cost_Notes):'','$'+addCost.toFixed(2)):''}
        <div style="${bdr}"></div>
        ...
        <div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--green-b);display:flex;gap:10px;flex-wrap:wrap;">
          <div style="flex:1;"><div style="color:var(--txt3);font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;">Payment</div>
            <div style="font-weight:700;font-size:13px;">${isPaid?'✅ '+esc(j.Payment_Method||'Paid'):'⏳ Unpaid'}</div></div>
          <div style="flex:1;"><div style="color:var(--txt3);font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;">Review</div>
            <div style="font-weight:700;font-size:13px;">${j.Review_Status||'—'}</div></div>
        </div>
      </div>
      ${!isPaid?`<div class="fg"><label class="fl">Payment Method</label><select class="fs" id="je-pm">${mOpts}</select></div>
...
async function submitJobEdit(btn, markPaid=false) {
  ...
  try {
    ...
    const getVal=id=>{ const el=$(id); if(!el||el.offsetParent===null) return undefined; return el.value; };
    ...
    const revStatus    = $('je-rev')?.value||undefined;
    ...
    Object.assign(j,{
      ...
      Pricing_Type:          pType,
      Flat_Rate:             flatRate!==undefined ? String(flatRate) : j.Flat_Rate,
      Surcharge:             String(currentSur),
      Additional_Cost:       String(currentAdd),
      Additional_Cost_Notes: addCostNotes||j.Additional_Cost_Notes,
      Total_Amount:          tot.toFixed(2)
    });
    if (revStatus!==undefined) j.Review_Status=revStatus;

    if (markPaid) {
      j.Payment_Status='Paid'; j.Payment_Method=method;
      const tod=today();
      const inv=S.financials.find(f=>f.Job_ID===jid);
      if (inv) { inv.Status='Paid'; inv.Payment_Method=method; inv.Paid_Date=tod; inv.Amount=tot.toFixed(2); }
      else S.financials.push({Invoice_ID:'INV'+Date.now(),Job_ID:jid,Client_ID:j.Client_ID,Amount:tot.toFixed(2),Status:'Paid',Payment_Method:method,Paid_Date:tod});
    } else {
      const inv=S.financials.find(f=>f.Job_ID===jid);
...
function confirmDeleteJob() {
  ...
  if (mdb) {
    mdb.innerHTML=`
      <div style="font-size:14px;color:var(--txt2);margin-bottom:16px;line-height:1.5;">
        Delete this <strong>${esc(j.Service)}</strong> job?<br><br>
...
async function submitQuickPaidFromSummary(btn) {
  ...
  try {
    ...
    j.Payment_Status='Paid'; j.Payment_Method=method;
    const inv=S.financials.find(f=>f.Job_ID===jid);
    if (inv) { inv.Status='Paid'; inv.Payment_Method=method; inv.Paid_Date=tod; inv.Amount=amt.toFixed(2); }
    else S.financials.push({Invoice_ID:'INV'+Date.now(),Job_ID:jid,Client_ID:j.Client_ID,Amount:amt.toFixed(2),Status:'Paid',Payment_Method:method,Paid_Date:tod});
    closeMo('m-job');
...
function jeCalc() {
  const jid=$('je-svc') ? S.jobModal : null; if(!jid) return;
  const j=getJob(jid); if(!j.Job_ID) return;
  const isFlat=$('je-pr-f')?.classList.contains('on');
  const preview=$('je-calc-preview'); if(!preview) return;
  const rate=parseFloat(S.biz.rate)||50;
  const tax=taxRate();
  let base=0;
  if (isFlat) { base=parseMoney($('je-flat')?.value||j.Flat_Rate); }
  else        { base=(parseFloat($('je-est-hrs')?.value||j.Estimated_Hours||0))*rate; }
  const sur=parseMoney($('je-sur')?.value||j.Surcharge);
  const add=parseMoney($('je-addcost')?.value||0);
  const sub=base+sur+add; const hst=sub*tax; const tot=sub+hst;
  preview.style.display='';
  preview.innerHTML=`
    <div style="display:flex;justify-content:space-between;"><span>Subtotal</span><span>$${sub.toFixed(2)}</span></div>
    ${hst>0?`<div style="display:flex;justify-content:space-between;"><span>HST (${Math.round(tax*100)}%)</span><span>$${hst.toFixed(2)}</span></div>`:''}
    <div style="display:flex;justify-content:space-between;font-weight:800;"><span>Total</span><span>$${tot.toFixed(2)}</span></div>`;
}

function jFU(v) {
  S.followUp=v;
  $('jfu-y')?.classList.toggle('on',v==='Yes');
  $('jfu-n')?.classList.toggle('on',v==='No');
}
...
async function submitQuickPaid(btn) {
  ...
  j.Payment_Status='Paid'; j.Payment_Method=method;
  const tod=today(); const amt=parseMoney(j.Total_Amount);
  const inv=S.financials.find(f=>f.Job_ID===jid);
  if (inv) { inv.Status='Paid'; inv.Payment_Method=method; inv.Paid_Date=tod; inv.Amount=amt.toFixed(2); }
  else S.financials.push({Invoice_ID:'INV'+Date.now(),Job_ID:jid,Client_ID:j.Client_ID,Amount:amt.toFixed(2),Status:'Paid',Payment_Method:method,Paid_Date:tod});
  closeMo('m-qpaid');
  // FIX: include ppAmt so the payment record gets the right amount even if the sheet total is stale
  if (!S.isDemo) gasCall({action:'markInvoicePaid',jobId:jid,method,ppAmt:amt.toFixed(2)});
  refreshData();
  showToast('💰 Payment recorded — $'+amt.toFixed(2));
  _isSaving=false;
}
...
function openCompleteModal(jid) {
  const j=getJob(jid); if(!j.Job_ID) return;
  const sd=j.Scheduled_Date;
  if (j.Scheduling_Type==='ASAP'&&!sd) { showToast('⚠️ Set a date before marking complete'); return; }
  ...
  if (mcb) {
    ...
    showMo('m-comp');
    requestAnimationFrame(()=>calcJobTotal());
  }
}
...
async function submitMarkPaid(btn) {
  if (_isSaving) return; _isSaving=true;
  if (btn) { btn.disabled=true; btn.textContent='Saving...'; }
  const jid=S.jobModal; const method=$('paid-method')?.value||'';
  const ppReason=$('pp-reason')?.value||'';
...
async function markRevRequested(jid) {
  ...
  refreshData();
  showToast('⭐ Review marked as Requested');
}
...
function openEditNotes() {
  ...
  if ($('m-notes-inp')) { $('m-notes-inp').value=c.Global_Notes||''; $('m-notes-inp').dataset.mode='clientnote'; }
  showMo('m-notes');
}
function openEditAccess() {
  const c=getCli(S.curCli); S.notesMeta='Access_Info';
  if ($('m-notes-t'))   $('m-notes-t').textContent='🔑 Access Info';
  if ($('m-notes-inp')) { $('m-notes-inp').value=c.Access_Info||''; $('m-notes-inp').dataset.mode='clientnote'; }
  showMo('m-notes');
}
function openEditFamily() {
  const c=getCli(S.curCli); S.notesMeta='Family_Details';
  if ($('m-notes-t'))   $('m-notes-t').textContent='👨‍👩‍👧 Family & Pets';
  if ($('m-notes-inp')) { $('m-notes-inp').value=c.Family_Details||''; $('m-notes-inp').dataset.mode='clientnote'; }
  showMo('m-notes');
}
...
async function saveNotes() {
  ...
  const origText=btn?btn.textContent:'';
  if (btn) { btn.disabled=true; btn.textContent='Saving...'; }
  const val=$('m-notes-inp')?.value.trim()||'';
  const mode=$('m-notes-inp')?.dataset.mode;
  closeMo('m-notes');
  if (mode==='jobnote') {
...
function openQuickNote(jid) {
  ...
  if ($('m-notes-inp')) { $('m-notes-inp').value=j.Job_Notes||''; $('m-notes-inp').dataset.mode='jobnote'; }
  showMo('m-notes');
}
...
function loadBizConfig() {
  ...
  if (!S.biz.logo||S.biz.logo==='') S.biz.logo=GLOBAL_LOGO;
  syncBizUI();
  updateHeaderBrand();
  const pb=$('pr-h');
...
function syncBizUI() {
  ...
  const emptyEl=$('logo-thumb-empty'); const imgEl=$('logo-thumb-img'); const removeBtn=$('logo-remove-btn');
  if (emptyEl&&imgEl) {
    if (logoSrc) { imgEl.src=logoSrc; imgEl.classList.remove('hidden'); emptyEl.style.display='none'; if(removeBtn) removeBtn.style.display='none'; }
    else         { imgEl.classList.add('hidden'); emptyEl.style.display=''; if(removeBtn) removeBtn.style.display='none'; }
  }
}
...
function updateHeaderBrand() {
  ...
  if (isDefault) nameEl.innerHTML='Supermom <span>for Hire</span>';
  else           nameEl.textContent=bizName;
  ...
  if (logoSource&&logoSource!==''&&!logoSource.startsWith('data:')) {
    ...
    logoEl.onload=function(){
      ...
      if (aspect>=2) { this.style.maxWidth='210px'; this.style.maxHeight='54px'; this.style.width='auto'; this.style.height='auto'; }
      else           { this.style.maxHeight='54px'; this.style.maxWidth='210px'; this.style.height='54px'; this.style.width='auto'; }
    };
    logoEl.src=logoSource; logoEl.classList.remove('hidden'); nameEl.style.display='none';
  } else {
    logoEl.classList.add('hidden'); logoEl.onload=null; nameEl.style.display='';
  }
}
...
function removeLogo() {
  S.biz.logo=''; syncBizUI(); updateHeaderBrand();
...
/*
 * ============================================================================
 * CHANGELOG — app.js v4.0
 * ============================================================================
 * BUG #1 FIXED: S.financials never loaded from server (loadAllData)
 *   → Added: S.financials = r.financials || []
 * BUG #2 FIXED: S.financials never restored from localStorage cache
 *   → Added: S.financials = p.financials || []
 * BUG #3 FIXED: Pre-payment label showed on ALL completed+paid jobs (jrHTML line ~777)
 *   → Changed: j.Job_Status !== 'Scheduled' → j.Job_Status === 'Scheduled'
 * BUG #4 FIXED: submitClient added new clients twice (dead code block lines 897-921)
 *   → Removed duplicate add block entirely
 * BUG #5 FIXED: gasCall not awaited in submitClient → real server ID never received
 *   → Added await to gasCall for addClient in submitClient and forceAddClient
 * BUG #6 FIXED: drainOfflineQueue never awaited gasCall → queue never drained
 *   → Added await to gasCall inside the drain loop
 * BUG #7 FIXED: Total_Amount never computed at job booking time
 *   → submitJob now calls getJobTotals and saves Total_Amount to jobData
 * BUG #8 FIXED: Scheduling_Type never saved at booking time
 *   → submitJob now saves Scheduling_Type: 'ASAP' or 'Scheduled'
 * BUG #9 FIXED: Duplicate getJobTotals function definition
 *   → First (dead) definition removed
 * BUG #10 FIXED: Pre-paid job completions created a duplicate payment record
 *   → markPaid now sends S.payNow && !isFullPrepay (not S.payNow || isFullPrepay)
 * BUG #11 FIXED: quickMarkPaid didn't send ppAmt to backend
 *   → Added ppAmt to both submitQuickPaid and submitQuickPaidFromSummary gasCall
 * ALSO: drainOfflineQueue now called on app init
 * ALSO: Lists from server (r.lists) now merged into S.lists on fresh load
 * ALSO: revStatus capture added to submitJobEdit so review edits persist
 * ALSO: pricingType added to submitJobEdit gasCall so Hourly/Flat switches persist
 * ============================================================================
 */
